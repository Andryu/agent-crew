...
| 塔アップグレード2段階 | **v1.5へ** | 配置済み塔の操作が2択化し常時Lv表示が要る。進化知覚への寄与は間接的。1次テストで「対抗手段が足りない」声が出たら追加（Gate B） |
...
6. **教えない導入**: 初回起動のみ（`storage.js` に `seenIntro`）。ウェーブ1前に吹き出し1つ「塔を置いて、ウェーブ開始」。ウェーブ1終了後の初回変異レポート冒頭に1行「群れは、あなたの守り方に合わせて変わります」。それ以外の説明UIは置かない
