...
## 人気TD分析からの追加仕様（2026-08-16、`docs/research/td-hits-analysis.md`）

テーゼ「進化が知覚されるか」を強めるもの、およびオーナー指摘「介入が薄いのは微妙」に応えるものだけをv1に追加する。

1. **次ウェーブ・プレビュー**: 配置フェーズ画面右に、次ウェーブの集団から5体をサンプリングして**見た目付き**で表示（`renderer.js` の個体描画を流用、静止）。見出し「次に来る群れ」。変異レポートを閉じた直後から表示
2. **能動スキル「発熱」**: ウェーブ中のみ使用可。レーンを1つ選ぶ→そのレーン上の全個体を3.0秒間 速度50%（cold耐性個体は75%）。クールダウン30秒。ボタンは画面下、残り秒数を表示。`game-state.js` に `useSkill(state, lane, now)` と `skillReadyAt` を追加
3. **塔アップグレード（分岐なし・2段階）**: 配置済み塔をタップ→「強化」。Lv2費用=初期費用×0.8、Lv3費用=初期費用×1.2。各Lvでダメージ×1.4、射程+0.3セル。売却額は投資総額の70%。`config.js` の `upgrade: { costMul: [0.8, 1.2], dmgMul: 1.4, rangeAdd: 0.3 }`。描画は塔の中心図形をLvに応じて1〜3重にする
4. **早出しボーナス**: 変異レポートを閉じてから10秒以内に「ウェーブ開始」を押すと +15 資金（表示「早出し +15」）。ウェーブ1は対象外
5. **撃破ジュース**: 撃破時に個体の塗り色で粒子6〜10個（0.4秒で消滅）＋撃破報酬の数字ポップ（0.6秒で上昇しつつフェード）。臓器到達時は画面端を赤く一瞬点滅（既存SEに合わせる）
...
v2候補（列挙のみ、v1では実装しない）: ボスウェーブ（5/10/15に最優秀個体の巨大版1体）／デイリーseed／ミューテーター／Lv3分岐／メタ解放。

工程への影響: 上記で **+2日（16日目安）**。バランス調整枠（旧D11-14）は圧縮しない。
...
- [ ] 次ウェーブ・プレビュー／能動スキル「発熱」／塔アップグレード2段階／早出しボーナス／撃破ジュース／初回導入 が仕様どおり動作する
...
## 16日の工程（目安、人気TD分析の追加分+2日込み）
- D1-3: 通常のTDとして遊べる状態（盤面・塔・敵・経済・15ウェーブ）
- D4-8: 進化・見た目直結・変異レポート・test.mjs／次ウェーブ・プレビュー・塔アップグレード・能動スキル
- D9-12: チャレンジリンク・アンケート・storage・SE／撃破ジュース・早出しボーナス・初回導入
- D13-16: バランス調整（オーナー実プレイ2回）・Pages配備・README
