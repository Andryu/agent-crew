...
| **Notion** | 人間がNotion上で直接生むもの＝**コメント・議論**。これらはBQ側に元データが存在せずNotionにしかない＝*Notionが原本*という意味（§6.4のとおりBQへはバックアップしない） | データ本体の原本。Notion上のファクト・インサイトは「写し（投影）」であり、消えてもBQから再生成できる。レポートページもBQから再生成できるため原本ではない |
...
- コメント・議論の扱いは§4の責任範囲表と§6.4のとおり（Notionにしか存在しない原本。BQへのバックアップはしない）
...
### 6.4 コメント・議論をBQにバックアップしない理由

当初「夜間にBQへバックアップする」と書いていたが、**Notion APIの制約で成立しないため取りやめる**。

- **解決済みコメントは取得できない** — コメント取得APIが返すのは未解決（open）のコメントのみ。解決済みは取得手段がない。取りこぼしが構造的に発生するものは「バックアップ」と呼べず、あると誤解される方が危険
- **全コメントを一覧するAPIがない** — 取得はページ（block_id）指定のみ。全部集めるには投影中の全ページを1件ずつ叩く必要があり、インサイト1,000ページなら毎晩1,000リクエスト（3件/秒制限で約6分）。投影の書込と同じレート枠を食い合う

> **代わりの方針 — そもそも失われないようにする**
> コメントが失われる心配は「Notionは作り直せる」という原則から来ていた。しかし投影は sync_state のページIDを使って**既存ページを更新する方式**であり、通常運用でページを作り直すことはない。したがってコメントは消えない。Notion全損のような事態はNotion自身の復元機能で対応する。
> そのうえで「議論の結論だけは構造化して残したい」ケースには、**結論をインサイトの description に書く**運用で対応する（人間の判断としてBQに入り、監査ログにも残る）。バックアップ機構を作るより、結論を残す運用のほうが実際に読み返される。
