...
本命の自社開発構成（§4〜§8, 案1）に加え、オーナー指示により **BigQuery × Notion × Claude による検証用構成（§10, 案2）** を厳密に併記する。判定は「対立ではなく直列」——案2をMVP-0（検証トラック）として先行させ、ゲートを越えたら案1に投資する。
...
## §10 代替案2: BigQuery × Notion × Claude 構成の徹底検討

オーナー指示: 「マスタデータソースはBQ、GUIをNotionで、Claude経由でファクトの抽出、インサイトの登録などをやるアーキテクチャ。厳しく、慎重に議論して進めて」

**結論を先に**: この構成は**SaaSプロダクトにはならない**が、§2のドメインモデルを丸ごと検証する**MVP-0（検証トラック）としては最速・最安**であり、案1と対立しない。
...
### 10.2 急所 — SSoTと同期の設計（フィールド所有権マトリクス）

2ストア＋人間＋AIの4者が同じデータを触る以上、双方向同期のドリフトが死因になり得る。**「どちらでも編集できる」フィールドを1つでも作った瞬間に破綻する。**
...
| 2 | **同期ドリフト**（Notion直編集・ルーチン失敗・部分適用） | 高 | 10.2の所有権マトリクス＋夜間の全件整合性照合（diff→自動修復＋監査記録） |
...
|---|---|---|
| 到達点 | マルチテナントSaaS | v1で単一組織のフル運用、v2でハイブリッド化し案1へ連続収斂（§10.6）。マルチテナントは最後まで不可 |
...
> 案2を MVP-0 として先行させるのは変わらないが、MVP-0 で終わりではない。案2は §10.6 の独自ロードマップ（MVP-0 → v1 → v2）を持ち、進み方は2通り:
> **(a) SaaS化を急ぐなら**: MVP-0 のゲート通過後すぐ案1へ投資
> **(b) 内部価値の最大化を優先するなら**: 案2を v1 まで育て、移行トリガー（§10.6）に当たった時点で案1へ
...
### 10.6 案2のロードマップ — この構成のまま、どこまで育てられるか
...
> ①マルチテナント不可（10.3-1） ②アフィニティマッピング不可（10.3-6） ③監査迂回は「検出まで」（10.3-3）。
...
- 2026-08-10: オーナー指摘（案2にv1がない）を受け §10.6 を新設 — 案2独自のロードマップ（MVP-0→v1→v2）、移行トリガー表、構造的限界の再掲。§10.5 の判定を「使い捨てMVP-0」から「独自ロードマップを持つ第2の本線」へ改訂
