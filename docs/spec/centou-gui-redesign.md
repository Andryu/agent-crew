...
- 議事録1本の取り込み（ファクト12件＋提案12件＋本文）≒ 約30リクエスト ≒ **10秒** → 問題なし
- 過去議事録50本のバックフィル ≒ 1,500リクエスト ≒ **8〜10分** → 夜間バッチなら可、対話的には無理
...
**想定データ量**: 議事録1本≒ファクト10〜15件。週2〜3本なら年間1,500〜2,000ファクト、インサイトは年間数十〜百件。Insights/Suggestionsは何年でも安全圏だが、**Factsを全量投影すると1〜3年で劣化ゾーンに入る**。「直近90日＋リンク済みのみ投影」はこの見積もりに対して妥当。全量のファクト閲覧はArtifact＋MCP（案B）またはSlack検索でBQに直接当てる。
...
> **「ArtifactからBQ更新はできる？」への回答**
> 直接は不可。ただし**BQを読み書きする自作MCPサーバをカスタムコネクタとして登録すれば、Artifactから window.claude.mcp 経由でBQの読み書きができる**。「MCPを作った方が良い？」の答えは Yes——MCPサーバがArtifact案を成立させる鍵。
...
| v1a（+2〜4週） | MCPサーバ縦スライス: list_suggestions＋review_suggestion のみ実装し、Artifact Inboxで実挙動検証 | Inbox承認がBQに反映され監査が残る |
| v1b（+1〜2ヶ月） | Artifact本実装（キャンバス・検索・内訳）。Notionはコメント・配布に役割縮小 | キャンバス経由の昇格が月N件 |
