...
ステータス: v2 実装済み（PR #177/#178）・**V3-1〜V3-3 および V3-5 実装済み**（V3-4 は次回棚卸しで実施）
起案: team-lead（Fable）
レビュー:
...
### V3-5. 編集単位の変更記録とリバート（オーナー承認済み・バックログ）

根拠: Prime Agent の `/refine` の内部規律（最小編集・トリガーと結果の記録・ID による
リバート）。verify-check のスプリント粒度より細かい「編集粒度」の追跡を、
git の既存機能に乗せて実現する:

- **1ルール編集 = 1コミット**の規約: pm-learned-rules.md・エージェント定義への
  lesson 由来の変更は、1件ごとに独立コミットとし、コミットメッセージに
  lesson_id を必ず含める（`propose-lesson-rules.sh` は既に対応済み。棚卸し・
  手動編集にも同じ規約を適用）
- **結果の記録**: 当該 lesson の verify-check 結果（verified / 再発リセット）が
  編集の成否記録を兼ねる
- **リバート**: ルールが逆効果と判明した場合、lesson_id でコミットを特定して
  `git revert`。台帳側は supersedes + dismissed で診断改訂（V3-1 の手順を流用）

新規の仕組みはほぼ不要（規約2行＋retro.md への追記のみ）。V3-4 と同じ棚卸し
サイクルで導入する。

### 導入しないと決めたもの（再掲・明確化）

- **スキルの A/B 並列評価・LLM judge 基盤**（Level 3）: 個人開発の規模では
  コストが効果に見合わない
- **ナレッジグラフ型記憶**（Zep/Graphiti 型）: 同上
- 再検討トリガー: 複数プロジェクト・複数人運用への拡大時のみ

### 実施順序と実施状況

| 項目 | 計画 | 実績 |
|------|------|------|
| V3-1 反証条件・診断改訂 | 次スプリント S | **実装済み**（retro.md ステップ2/2.7、lessons.sh の supersedes 存在チェック） |
| V3-2 信頼境界 | 次スプリント S | **実装済み**（owner_approved、`lessons.sh list-rule-candidates` への抽出一元化） |
| V3-3 差分棚卸し・lint | 次スプリント S | **実装済み**（retro.md 棚卸し手順の30%閾値、月次 Routine の lint 項目） |
| V3-4 助言化 | 次回棚卸し | 未実施（次回棚卸しで `propose-lesson-rules.sh` 4箇所と併せて実施） |
| V3-5 編集単位リバート | 次回棚卸し | **前倒しで実装済み**（retro.md 棚卸し節に「1ルール編集=1コミット」規約を明記）。次回棚卸しタスクの計画時に重複起票しないこと |

**実装時に判明した追加の設計判断（V3-2 の一元化）**

当初案は retro.md と `propose-lesson-rules.sh` の**両方に**同等の jq フィルタを書く
方針だったが、Sora のレビューで正規化ロジックの非対称（`ssh://` 形式未対応・
origin 未設定時のフォールバック欠如）が実測で検出された。加えて実装中に、
priority 閾値（3 vs 4）と status 条件（2値 vs 4値）の非対称も判明した。
とくに後者は `lessons.sh add` の既定 status（`proposed`）を retro.md 側のクエリが
弾いており、**既定手順で記録した lesson が書き出し対象から全て漏れる潜在バグ**だった。

対処として、抽出条件の実装を `scripts/lessons.sh list-rule-candidates` に一元化し、
retro.md と `propose-lesson-rules.sh` の双方がそれを呼ぶ形に変更した。
クエリの複製を禁止する旨を両方に明記し、9件のテスト（origin 3形式の parity・
origin 未設定・レガシーエントリ・proposed status・除外の可視化ほか）を追加している。
