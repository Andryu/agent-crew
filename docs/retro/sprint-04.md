...
|-----------|---------------|------|
| agent-crew-sprint-04-process-001 | 4 | 保留（gh コマンド未インストールのため Issue 化不可） |

保留理由: `gh` コマンドがこの実行環境にインストールされていないため Issue 化を実施できなかった。環境に `gh` が導入された時点で以下の内容で Issue を作成すること:

- タイトル: `[lesson] Alex が Bash ツールを持たず queue.sh done/handoff を Yuki が代行`
- ラベル: `priority-medium`, `retro`, `lessons-learned`
- body: lesson ID `agent-crew-sprint-04-process-001` の description / evidence / action を記載
...
1. **Alex の Bash ツール付与を最優先で検討する**
   スプリントの自律性を高めるうえで最も効果が高い。architect.md のツール定義を確認し、Bash を追加できるか Yuki に判断を求める。
