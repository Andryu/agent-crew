...
> **注意:** Antigravity は SubagentStop hook 未対応のため、パイプラインの完全自動化は動作しません。各エージェントは完了時に `--- NEXT STEP ---` ブロックで次のコマンドを提示するので、オーナーがそれをコピーして次エージェントを呼び出してください。

インストール後に `queue.sh` が `.agent/scripts/queue.sh` として配置され、タスクキュー管理が使えます。
...
| グローバル配置 | ✅ | ✅ |
| プロジェクト配置 | ✅ | ✅ |
| 自動パイプライン（hook） | ✅ | 部分対応（STDOUT 提示） |
| Slack 通知 | ✅ | 部分対応（curl 手動実行） |
| タスクキュー管理 | ✅ | 対応（`.agent/scripts/queue.sh`） |
