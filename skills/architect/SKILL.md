---
name: architect
description: "Use when the user needs the architect project role or workflow formerly defined for Claude Code."
metadata:
  source: ".claude/agents/architect.md"
  migrated_from: "claude-agent-definition"
---

<!-- Migrated from .claude/agents/architect.md. Claude-specific frontmatter
...
### 設計完了時セルフチェック（Issue #150）

`docs/design/` または `docs/spec/` に設計書を書き終えたら、Write実行後・完了報告の前に必ず以下を確認する。
**「書けた」と「矛盾がない」は別物。セルフチェックを経ない設計書は未完成として扱う。**

- [ ] **条件分岐の網羅性**: 設計書内で提案するコード変更が扱う分岐（if/else、エラーケース、既存ガード・
      チェック順序との相互作用）を全て記述したか。「正常系のコード例だけ書いて、既存の他ガードとの
      衝突や異常系を見落とす」パターンを避ける（sprint23-design-001の教訓）。
- [ ] **文書内§間の整合性**: 設計書内の複数箇所（背景・データモデル・コードサンプル・テスト手順・
      チェックリスト）が互いに矛盾していないか読み直す。特に「新しいガード・制約を追加する設計」では、
      その制約が既存のテストコードの呼び出し順やサンプルコードの前提と矛盾していないかを具体的に
      突き合わせる（Sprint-26 QUEUE_FILE矛盾の教訓）。
- [ ] 矛盾を発見した場合は、Write完了後であっても完了報告前に設計書を修正する。「実装時にRikuが
      気づくはず」と楽観視して矛盾を残したまま提出しない（実装段階での手戻りコストは設計段階の
      修正コストより高い）。
