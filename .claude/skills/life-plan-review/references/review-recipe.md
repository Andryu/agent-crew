...
CREATE TABLE ("回" TITLE, "見直し日" DATE,
  ...
  "積立月額" NUMBER, "目標積立月額" NUMBER,
  "目標純資産" NUMBER, "目標との差分" NUMBER,
  "進捗率" FORMULA('prop("純資産") / prop("目標純資産")'),
  "メモ" RICH_TEXT)
```
- 列はテーマに合わせて足し引きしてよい（NISA/iDeCo を分けず「投資信託」「株式」等の保有実態に合わせるなど）。
...
- **`積立月額`（実績）と `目標積立月額` を毎回記録する**と、「目標ペースに乗っているか」がひと目で分かる（資産形成見直しの肝）。
- **`進捗率` は FORMULA で自動計算**（`純資産 / 目標純資産`）。FORMULA 列は `notion-create-database` の DDL で定義する（プロパティ名は実際の純資産列に合わせる）。Notion の数値書式でパーセント表示にすると読みやすい。
- ビュー（`notion-create-view`）：
  - **推移テーブル（主）**：`SORT BY "見直し日" ASC`。純資産・進捗率・目標との差分の伸びを上から下へ追える。
  - チャートは `CHART line` ＋ `GROUP BY "見直し日"` が必須（x軸の指定がないと作成に失敗する）。使えなければテーブルのみでよい（UIで後からチャート化も可能）。
