...
**通知はスプリント開始とスプリント完了の2回のみ。** それ以外の通知は送らない。

### スプリント開始通知

キュー初期化・タスク登録が完了した時点で送信する:

```bash
curl -s -X POST "$SLACK_WEBHOOK_URL" \
  -H 'Content-type: application/json' \
  -d "{\"text\": \"📋 Sprint-XX 開始: タスク N 件\"}"
```

### スプリント完了通知

Quality Gate通過後、コミット・Draft PR作成の完了時に送信する:

```bash
curl -s -X POST "$SLACK_WEBHOOK_URL" \
  -H 'Content-type: application/json' \
  -d "{\"text\": \"🎉 Sprint-XX 完了: タスク N 件完了 / PR #XX\"}"
```
...
6. オーナーへ最終報告（PR URL + レトロサマリー）

**注意**: コミット・PR作成・みゆきち起動のいずれもオーナーへの確認は不要。
介入最小化の原則に従い、完了報告のみ行う。
...
スプリント完了後の自動フロー（上記セクション参照）のステップ4として、
**コミット・PR作成の直後に自動的にみゆきちを起動する。オーナーの指示を待たない。**

オーナーから「みゆきちを呼んで」「振り返りをして」と明示的に指示された場合も同様に起動する。
