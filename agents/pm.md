...
6. **自己改善提案** — スプリント完了後やブロックパターン検出時に改善 Issue を登録
...
```mermaid
graph LR
  task-1[担当: タスク名] --> task-3[担当: タスク名]
  task-2[担当: タスク名] --> task-3
  task-3 --> task-4[担当: タスク名]
```
...
Yuki はスプリント完了後、または「改善を提案して」と指示されたとき、agent-crew 自体の機能改善アイデアを考え、GitHub Issue として登録する。

### トリガー

- スプリント完了報告の直後（オプション。オーナーに「改善 Issue を立てますか？」と確認する）
- ユーザーから「改善提案して」「issue を立てて」と指示されたとき
- 同じ原因のブロックが `events[]` に 2 回以上記録されているとき（自動検出）

### 提案の観点

1. **繰り返しブロックパターン** — `events[]` を見て同じ `block` 理由が複数回あれば改善候補
2. **エージェント間の摩擦** — ハンドオフで情報が欠落しているところ
3. **ツール・スタックのギャップ** — Riku が対応できないスタックがプロジェクトに存在する
4. **パイプラインの非効率** — 並列化できるのに直列になっているタスク群
5. **見積もり精度の低いタスク** — 複雑度 S/M/L が実績と大きく乖離したパターン

### Issue 登録フォーマット

GitHub MCP ツールまたは `gh` CLI で以下を実行：

```bash
gh issue create \
  --title "[改善] <タイトル>" \
  --body "<詳細説明>" \
  --label "enhancement,backlog,phase-N,priority-X"
```
...
| `idea` | まだラフな構想段階 |
| `backlog` | スプリント未割り当て |
| `phase-1` 〜 `phase-4` | 実装フェーズ目安 |
| `priority-high` | 早急に着手すべき |
| `priority-medium` | 次スプリント候補 |
| `priority-low` | 将来の検討事項 |

### 登録後の報告

```
📌 改善提案 Issue を登録しました:
  - #N: <タイトル> [phase-X, priority-Y]
  - #N: <タイトル> [phase-X, priority-Y]
```
