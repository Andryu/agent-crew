...
5. 配置済みの塔をタップすると「強化」「売る」のパネルが出る。強化は2段階（Lv3が上限）で、Lvごとにダメージ×1.4・射程+0.3になる。タイトルの「あそびかた」（プレイ中はHUDの「?」）で操作と敵の見た目の凡例をいつでも確認できる。
6. ウェーブ15をクリアすると「防衛完了」。ライフが0になると「あなたを倒した群れ」としてゲームオーバー。到達ウェーブ数がスコア（自己ベストはlocalStorageに保存）。
7. 結果画面から「この条件で挑戦状を送る」で同じseed/初期資金のURLをコピーでき、そのURLを開いた相手は同じ条件で群れの進化を再現できる（決定的乱数）。
8. 結果画面から「アンケートに答える」で任意のフィードバックを送れる（回答は端末内のlocalStorageに溜まり、「結果をコピー」でJSONとして取り出せる）。
