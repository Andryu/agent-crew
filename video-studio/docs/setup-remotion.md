...
動画クリップのフレーム内でキャラが正方形の枠の中を大きく動く場合、9:16に引き伸ばすと横方向が大きくクロップされ、
キャラが画面外に見切れることがあった（実測: 全身版jumpモーションで実際に発生）。そのため動画レイヤーの `objectFit` は
`contain`（縦横比を保ったまま全体を収める。画像レイヤーは `cover` のまま変更なし）にしている。
containで生じる余白はCutのベース背景（`paperTextureStyle("#ffffff")`、白＋うっすら紙テクスチャ）と自然に馴染む前提。
